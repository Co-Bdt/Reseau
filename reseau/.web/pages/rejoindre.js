...
export function Fragment_853c7dd2c4a758640fb81185d7b8e927 () {
  const reflex___state____state__reseau___common___base_state____base_state = useContext(StateContexts.reflex___state____state__reseau___common___base_state____base_state)
  const { resolvedColorMode } = useContext(ColorModeContext)
  ...
  const ref_profile_img = useRef(null); refs['ref_profile_img'] = ref_profile_img;
    ...
    const handleSubmit_0f9cce25f2bba98cf7287bd47fb7386b = useCallback((ev) => {
        const $form = ev.target
        ev.preventDefault()
        const form_data = {...Object.fromEntries(new FormData($form).entries()), ...{"username": getRefValue(refs['ref_username']), "profile_img": getRefValue(refs['ref_profile_img']), "email": getRefValue(refs['ref_email']), "confirm_password": getRefValue(refs['ref_confirm_password']), "password": getRefValue(refs['ref_password'])}}
    ...
    const handleSubmit_7d84dc2110d3e40957d9d30b2903165e = useCallback((ev) => {
        const $form = ev.target
        ev.preventDefault()
        const form_data = {...Object.fromEntries(new FormData($form).entries()), ...{"username": getRefValue(refs['ref_username']), "profile_img": getRefValue(refs['ref_profile_img']), "email": getRefValue(refs['ref_email']), "confirm_password": getRefValue(refs['ref_confirm_password']), "password": getRefValue(refs['ref_password'])}}
  ...
  <RadixFormRoot className={`Root `} css={{"width": "100%", "margin": "0"}} onSubmit={handleSubmit_0f9cce25f2bba98cf7287bd47fb7386b}>
  ...
  {``}
  {reflex___state____state__reseau___common___base_state____base_state__reseau___pages___registration____registration_state.cities_as_str.map((item, index_ba92bb90960b5d4e) => (
  <RadixThemesSelect.Item key={index_ba92bb90960b5d4e} value={item}>
  {item}
  ...
  <RadixFormRoot className={`Root `} css={{"width": "100%", "margin": "0"}} onSubmit={handleSubmit_7d84dc2110d3e40957d9d30b2903165e}>
  <RadixThemesFlex align={`start`} className={`rx-Stack`} css={{"width": "100%", "minHeight": "85vh"}} direction={`column`} justify={`center`} gap={`3`}>
  <RadixThemesFlex align={`start`} className={`rx-Stack`} css={{"width": "100%"}} direction={`row`} justify={`center`} gap={`3`}>
  <ReactDropzone accept={{"image/png": [".png"], "image/jpeg": [".jpg", ".jpeg"]}} id={`profile_img`} multiple={false} onDrop={(_files) => addEvents([Event("reflex___state____state.reseau___common___base_state____base_state.reseau___pages___registration____registration_state.handle_upload", {files:_files,upload_id:`profile_img`}, "uploadFiles")], (_files), {})} ref={ref_profile_img}>
  {({ getRootProps, getInputProps }) => (
...
export default function Component() {
  ...
  <Fragment_853c7dd2c4a758640fb81185d7b8e927/>
