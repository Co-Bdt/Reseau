...
export function Fragment_01467693e74953287f70d56e3ada9d48 () {
  const reflex___state____state__reseau___common___base_state____base_state = useContext(StateContexts.reflex___state____state__reseau___common___base_state____base_state)
  const { resolvedColorMode } = useContext(ColorModeContext)
  ...
  const ref_profile_img = useRef(null); refs['ref_profile_img'] = ref_profile_img;
    ...
    const handleSubmit_fe35abb7001048b96f71d6e8446f4414 = useCallback((ev) => {
        const $form = ev.target
        ev.preventDefault()
        const form_data = {...Object.fromEntries(new FormData($form).entries()), ...{"confirm_password": getRefValue(refs['ref_confirm_password']), "username": getRefValue(refs['ref_username']), "profile_img": getRefValue(refs['ref_profile_img']), "email": getRefValue(refs['ref_email']), "password": getRefValue(refs['ref_password'])}}
    ...
    const handleSubmit_548a76233e3bcfce42312068d07c9bdc = useCallback((ev) => {
        const $form = ev.target
        ev.preventDefault()
        const form_data = {...Object.fromEntries(new FormData($form).entries()), ...{"confirm_password": getRefValue(refs['ref_confirm_password']), "username": getRefValue(refs['ref_username']), "profile_img": getRefValue(refs['ref_profile_img']), "email": getRefValue(refs['ref_email']), "password": getRefValue(refs['ref_password'])}}
  ...
  <RadixFormRoot className={`Root `} css={{"width": "100%", "margin": "0"}} onSubmit={handleSubmit_fe35abb7001048b96f71d6e8446f4414}>
  ...
  {``}
  {reflex___state____state__reseau___common___base_state____base_state__reseau___pages___registration____registration_state.cities_as_str.map((item, index_3354e3c2b608286b) => (
  <RadixThemesSelect.Item key={index_3354e3c2b608286b} value={item}>
  {item}
  ...
  <RadixFormRoot className={`Root `} css={{"width": "100%", "margin": "0"}} onSubmit={handleSubmit_548a76233e3bcfce42312068d07c9bdc}>
  <RadixThemesFlex align={`start`} className={`rx-Stack`} css={{"width": "100%", "minHeight": "85vh"}} direction={`column`} justify={`center`} gap={`3`}>
  <RadixThemesFlex align={`start`} className={`rx-Stack`} css={{"width": "100%"}} direction={`row`} justify={`center`} gap={`3`}>
  <ReactDropzone accept={{"image/png": [".png"], "image/jpeg": [".jpg", ".jpeg"]}} id={`profile_img`} multiple={false} onDrop={(_files) => addEvents([Event("reflex___state____state.reseau___common___base_state____base_state.reseau___pages___registration____registration_state.handle_upload", {files:_files,upload_id:`profile_img`}, "uploadFiles")], (_files), {})} ref={ref_profile_img}>
  {({ getRootProps, getInputProps }) => (
...
export default function Component() {
  ...
  <Fragment_01467693e74953287f70d56e3ada9d48/>
