...
export function Fragment_a76c4163ba1ecda9b3d517794a915e50 () {
  ...
  const ref_profile_img = useRef(null); refs['ref_profile_img'] = ref_profile_img;
    ...
    const handleSubmit_86294d897405e0aaaedf973ad8359ffa = useCallback((ev) => {
        const $form = ev.target
        ev.preventDefault()
        const form_data = {...Object.fromEntries(new FormData($form).entries()), ...{"confirm_password": getRefValue(refs['ref_confirm_password']), "email": getRefValue(refs['ref_email']), "profile_img": getRefValue(refs['ref_profile_img']), "password": getRefValue(refs['ref_password']), "username": getRefValue(refs['ref_username'])}}
    ...
    const handleSubmit_c3ce648fbe59e0bad2968267993947dd = useCallback((ev) => {
        const $form = ev.target
        ev.preventDefault()
        const form_data = {...Object.fromEntries(new FormData($form).entries()), ...{"confirm_password": getRefValue(refs['ref_confirm_password']), "email": getRefValue(refs['ref_email']), "profile_img": getRefValue(refs['ref_profile_img']), "password": getRefValue(refs['ref_password']), "username": getRefValue(refs['ref_username'])}}
  ...
  <RadixFormRoot className={`Root `} css={{"width": "100%", "margin": "0"}} onSubmit={handleSubmit_86294d897405e0aaaedf973ad8359ffa}>
  ...
  {``}
  {reflex___state____state__reseau___common___base_state____base_state__reseau___pages___registration____registration_state.cities_as_str.map((item, index_9f59c195340917f2) => (
  <RadixThemesSelect.Item key={index_9f59c195340917f2} value={item}>
  {item}
  ...
  <RadixFormRoot className={`Root `} css={{"width": "100%", "margin": "0"}} onSubmit={handleSubmit_c3ce648fbe59e0bad2968267993947dd}>
  <RadixThemesFlex align={`start`} className={`rx-Stack`} css={{"width": "100%", "minHeight": "85vh"}} direction={`column`} justify={`center`} gap={`3`}>
  <RadixThemesFlex align={`start`} className={`rx-Stack`} css={{"width": "100%"}} direction={`row`} justify={`center`} gap={`3`}>
  <ReactDropzone accept={{"image/png": [".png"], "image/jpeg": [".jpg", ".jpeg"]}} id={`profile_img`} multiple={false} onDrop={(_files) => addEvents([Event("reflex___state____state.reseau___common___base_state____base_state.reseau___pages___registration____registration_state.handle_upload", {files:_files,upload_id:`profile_img`}, "uploadFiles")], (_files), {})} ref={ref_profile_img}>
  {({ getRootProps, getInputProps }) => (
...
export default function Component() {
  ...
  <Fragment_a76c4163ba1ecda9b3d517794a915e50/>
