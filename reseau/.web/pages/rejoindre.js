...
export function Fragment_5f41c7f56514f43c99d63e58e7f85c7b () {
  const reflex___state____state__reseau___common___base_state____base_state = useContext(StateContexts.reflex___state____state__reseau___common___base_state____base_state)
  const { resolvedColorMode } = useContext(ColorModeContext)
  ...
  const ref_profile_img = useRef(null); refs['ref_profile_img'] = ref_profile_img;
    ...
    const handleSubmit_031edab5eca466526aed7010a9270631 = useCallback((ev) => {
        const $form = ev.target
        ev.preventDefault()
        const form_data = {...Object.fromEntries(new FormData($form).entries()), ...{"email": getRefValue(refs['ref_email']), "profile_img": getRefValue(refs['ref_profile_img']), "password": getRefValue(refs['ref_password']), "username": getRefValue(refs['ref_username']), "confirm_password": getRefValue(refs['ref_confirm_password'])}}
    ...
    const handleSubmit_123cef0cb5f5ddfb8ca135d622fe317b = useCallback((ev) => {
        const $form = ev.target
        ev.preventDefault()
        const form_data = {...Object.fromEntries(new FormData($form).entries()), ...{"email": getRefValue(refs['ref_email']), "profile_img": getRefValue(refs['ref_profile_img']), "password": getRefValue(refs['ref_password']), "username": getRefValue(refs['ref_username']), "confirm_password": getRefValue(refs['ref_confirm_password'])}}
  ...
  <RadixFormRoot className={`Root `} css={{"width": "100%", "margin": "0"}} onSubmit={handleSubmit_031edab5eca466526aed7010a9270631}>
  ...
  {``}
  {reflex___state____state__reseau___common___base_state____base_state__reseau___pages___registration____registration_state.cities_as_str.map((item, index_08afcf926f432f2f) => (
  <RadixThemesSelect.Item key={index_08afcf926f432f2f} value={item}>
  {item}
  ...
  <RadixFormRoot className={`Root `} css={{"width": "100%", "margin": "0"}} onSubmit={handleSubmit_123cef0cb5f5ddfb8ca135d622fe317b}>
  <RadixThemesFlex align={`start`} className={`rx-Stack`} css={{"width": "100%", "minHeight": "85vh"}} direction={`column`} justify={`center`} gap={`3`}>
  <RadixThemesFlex align={`start`} className={`rx-Stack`} css={{"width": "100%"}} direction={`row`} justify={`center`} gap={`3`}>
  <ReactDropzone accept={{"image/png": [".png"], "image/jpeg": [".jpg", ".jpeg"]}} id={`profile_img`} multiple={false} onDrop={(_files) => addEvents([Event("reflex___state____state.reseau___common___base_state____base_state.reseau___pages___registration____registration_state.handle_upload", {files:_files,upload_id:`profile_img`}, "uploadFiles")], (_files), {})} ref={ref_profile_img}>
  {({ getRootProps, getInputProps }) => (
...
export default function Component() {
  ...
  <Fragment_5f41c7f56514f43c99d63e58e7f85c7b/>
